...
export function SwimlaneNode({ id, data, selected }: NodeProps) {
  ...
  const hasValidationError = Boolean(nodeData.validationError)
  const backgroundColor = nodeData.style?.backgroundColor ?? (nodeData.variant === 'decision' ? '#E8F8F4' : laneStyle.bg)
  const borderColor = hasValidationError ? '#EF4444' : nodeData.style?.borderColor ?? (selected ? '#8B5CF6' : nodeData.variant === 'decision' ? '#CFEDE6' : laneStyle.border)
    ...
    <div className="relative" style={{ width: SWIMLANE_NODE_WIDTH }}>
      ...
      <div className={isDecision ? 'flex justify-center py-2' : ''}>
        <div
          ...
          } ${hasValidationError ? 'ring-4 ring-red-500/20' : selected ? 'ring-4 ring-[#6B46F2]/18' : ''} ${selected && hasValidationError ? 'outline outline-2 outline-offset-2 outline-[#8B5CF6]' : ''}`}
        ...
        >
          ...
          {hasValidationError ? (
            <div className={`absolute -right-2 -top-2 flex h-6 w-6 items-center justify-center rounded-full border border-red-200 bg-white text-red-500 shadow-sm ${contentClass}`} title={nodeData.validationError} role="img" aria-label={nodeData.validationError}>
