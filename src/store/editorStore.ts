...
import { autoLayoutGraph, estimateNodeHeight, laneColors, laneIdFromNodeCenter, nodeGapY, nodeStartY, nodeXForLane, normalizeLanes } from "../lib/graph/autoLayout"
...
import { maxOutgoingEdgesForSource } from "../lib/graph/orthogonalRouting"
...
const starterGraph = initialParse.ok ? autoLayoutGraph(initialParse.graph) : fallbackGraph
...
export const setEditorReadOnly = (value: boolean) => {
  readOnlyMode = value
}

const loadStoredDraft = (): StoredDraft | null => {
  if (!hasWindow() || !currentDiagramId) return null
  try {
    const raw = window.localStorage.getItem(diagramStorageKey(currentDiagramId))
    if (!raw) return null
...
const persistDraft = (graph: GraphModel, mermaidSource: string) => {
  if (!hasWindow() || !currentDiagramId) return
  try {
    window.localStorage.setItem(diagramStorageKey(currentDiagramId), JSON.stringify({ graph, mermaidSource }))
  } catch {
    // Persistence is best-effort and should never block editing.
  }
}

const isLayoutGridPosition = (graph: GraphModel, node: GraphModel["nodes"][number]): boolean => {
  const lanes = normalizeLanes(graph.lanes)
  const lane = lanes.find((item) => item.id === node.data.laneId)
  if (!lane || Math.abs(node.position.x - nodeXForLane(lane)) >= 1) return false

  const row = Math.round((node.position.y + estimateNodeHeight(node) / 2 - nodeStartY) / nodeGapY)
  const expectedY = nodeStartY + row * nodeGapY - estimateNodeHeight(node) / 2
  return Math.abs(node.position.y - expectedY) < 1
}

const relayoutStoredDraft = (draft: StoredDraft | null): GraphModel | undefined => {
  const graph = draft?.graph
  if (!graph || !graph.nodes.every((node) => isLayoutGridPosition(graph, node))) return graph
  return autoLayoutGraph(graph)
}

const storedDraft = loadStoredDraft()
const initialGraph = relayoutStoredDraft(storedDraft) ?? starterGraph
const initialMermaidSource = storedDraft?.mermaidSource ?? defaultMermaid

const loadDiagramData = (id: string): (StoredDraft & { past?: GraphModel[]; future?: GraphModel[] }) | null => {
  if (!hasWindow()) return null
  try {
    const raw = window.localStorage.getItem(diagramStorageKey(id))
    if (!raw) return null
    return JSON.parse(raw) as StoredDraft & { past?: GraphModel[]; future?: GraphModel[] }
  } catch {
    return null
  }
}

const saveDiagramData = (id: string, data: StoredDraft & { past?: GraphModel[]; future?: GraphModel[] }) => {
  if (!hasWindow()) return
  try {
...
type EditorStore = {
  ...
  loadExternalDiagram: (graph: GraphModel, mermaidSource: string, diagramId: string | null) => void
  clearSelection: () => void
...
const commitGraph = (
  ...
  if (currentDiagramId) {
    saveDiagramData(currentDiagramId, {
      graph: nextGraph,
      mermaidSource: nextMermaidSource,
      past,
      future: [],
    })
    saveQueue.enqueue(currentDiagramId, nextGraph, nextMermaidSource)
  }
  set({
    graph: nextGraph,
    mermaidSource: nextMermaidSource,
...
export const useEditorStore = create<EditorStore>((set, get) => ({
  ...
  loadExternalDiagram: (graph, mermaidSource, diagramId) => {
  ...
  addEdge: (input) => {
    ...
    const sourceNode = graph.nodes.find((node) => node.id === input.source)
    if (!sourceNode) return null
    const outgoingCount = graph.edges.filter((edge) => edge.source === input.source).length
    if (outgoingCount >= maxOutgoingEdgesForSource(sourceNode.data.variant)) return null
  ...
  reverseEdgeDirection: (edgeId) => {
    const graph = get().graph
    const edgeToReverse = graph.edges.find((edge) => edge.id === edgeId)
    if (!edgeToReverse) return
    const nextSourceNode = graph.nodes.find((node) => node.id === edgeToReverse.target)
    if (!nextSourceNode) return
    const outgoingCount = graph.edges.filter((edge) => edge.id !== edgeId && edge.source === edgeToReverse.target).length
    if (outgoingCount >= maxOutgoingEdgesForSource(nextSourceNode.data.variant)) return
    commitGraph(set, get, {
      ...graph,
      edges: graph.edges.map((edge) =>
        edge.id === edgeId
  ...
  undo: () => {
    ...
    if (currentDiagramId) {
      saveDiagramData(currentDiagramId, { graph: previous, mermaidSource, past, future })
    }
  ...
  redo: () => {
    ...
    if (currentDiagramId) {
      saveDiagramData(currentDiagramId, { graph: next, mermaidSource, past, future })
    }
  ...
  loadDiagram: (id: string) => {
    ...
    if (data) {
      const graph = relayoutStoredDraft({ graph: data.graph, mermaidSource: data.mermaidSource }) ?? data.graph ?? starterGraph
      const mermaidSource = data.mermaidSource ?? defaultMermaid
      set({
        graph: sanitizeGraph(safeCloneGraph(graph)),
        mermaidSource,
        past: data.past ?? [],
        future: data.future ?? [],
        canUndo: (data.past?.length ?? 0) > 0,
        canRedo: (data.future?.length ?? 0) > 0,
        selectedNodeId: null,
        selectedEdgeId: null,
        selectedLaneId: null,
        laneDeleteMessage: null,
      ...
      set({
        graph: sanitizeGraph(safeCloneGraph(starterGraph)),
